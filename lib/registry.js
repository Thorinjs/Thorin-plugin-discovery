  ...
  * */
    ...
    fetch(url, fetchOpt)
      ...
      .catch((e) => {
        ...
        if(e instanceof SyntaxError) {
          return done(thorin.error('REGISTRY.DISPATCH', 'Could not parse service response data.'));
        }
        if(e.name && e.name.indexOf('Thorin') === 0) {
          return done(e);
        }
        logger.warn(`Service [${serviceNode.name} - ${url}] returned with an error: ${statusCode}: ${e.message}`);
        return done(thorin.error('REGISTRY.DISPATCH', 'Coult not contact service', e, statusCode));
      });
  }
  ...
  function doRetryDispatch(serviceName, fetchOpt, current, max, done) {
    ...
    doDispatch(serviceNode, fetchOpt, (err, res) => {
      ...
      doRetryDispatch(serviceName, fetchOpt, current, max);
    });
  }
