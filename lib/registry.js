...
const REGISTRY_EVENT = 'discovery.registry';
...
module.exports = function (thorin, opt, pluginObj) {
  ...
  /**
   * Manually set the registry services/URLs from the config.
   * */
  pluginObj.setRegistry = (data) => {
    if (typeof data !== 'object' || !data) return false;
    Object.keys(data).forEach((serviceName) => {
      let urls = (data[serviceName] instanceof Array ? data[serviceName] : [data[serviceName]]);
      ...
      for (let i = 0, len = urls.length; i < len; i++) {
        ...
        if (typeof t === 'string') {
          ...
          let item = {
            name: serviceName + '-' + i,
            host: t.hostname,
            proto: t.protocol.split(':')[0],
            port: parseInt(t.port || (t.protocol === 'http:' ? 80 : 443)),
            path: t.pathname,
            tags: [],
            env: thorin.env,
            static: true,
          ...
          item.sid = thorin.util.sha1(item.id);
          REGISTRY[serviceName].push(item);
          serviceKey = opt.token;
          if (serviceKey.indexOf('-') !== -1) {
            serviceKey = serviceKey.split('-')[1];
          }
        } else if (typeof t === 'object' && t) {
          REGISTRY[serviceName].push(t);
        }
      }
    });
    thorin.dispatcher.emit(REGISTRY_EVENT, REGISTRY);
  }
  ...
  pluginObj.refresh = function RefreshRegistry(done) {
    ...
    if (actionPayload.version == null) delete actionPayload.version;
    if (actionPayload.timeout == null) delete actionPayload.timeout;
    fetcherObj
      ...
      .then((r) => {
        ...
        if (r.result instanceof Array) {
          for (let i = 0, len = r.result.length; i < len; i++) {
            ...
            if (opt.registry && opt.registry[item.type]) { // Check if we should add the item, if it's defined as static.
              canAdd = false;
              NEW_REGISTRY[item.type] = REGISTRY[item.type];
            } else if (typeof NEW_REGISTRY[item.type] === 'undefined') {
      ...
      .catch((e) => {
        logger.warn(`Could not refresh registry: ${e.code || 'ERROR'}`);
        if (typeof done === 'function') {
          logger.warn(e);
          done(e);
        }
  ...
  function doDispatch(serviceNode, fetchOpt, done) {
    ...
    fetch(url, fetchOpt)
      ...
      .then((result) => {
        if (typeof result === 'object' && result && typeof result.error === 'object' && result.error) {
          ...
          if (result.error.fields) err.fields = result.error.fields;
