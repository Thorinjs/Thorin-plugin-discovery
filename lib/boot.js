...
module.exports = function boot(thorin, opt, pluginObj) {
  ...
  pluginObj.run = function (next) {
    ...
    if (tObj) {
      ...
      if (opt.service) {
        ...
        if (!opt.service.path) {
          opt.service.path = (transportConfig.actionPath instanceof Array ? transportConfig.actionPath[0] || '' : transportConfig.actionPath || '');
        }
      }
    }
    if (opt.service) {
    ...
    calls.push((done) => {
      ...
      if (opt.registry || opt.refresh === false) return done(); //custom registry
      if (opt.delay === 0) {
        return pluginObj.refresh(done);
      }
      done();
      setTimeout(() => {
        pluginObj.refresh((e) => {
          if (e) {
            logger.warn(`Could not register in the cluster`);
          }
        });
      }, opt.delay);
    });
    ...
    async.series(calls, (e) => {
      ...
      if (opt.registry) {
        pluginObj.setRegistry(opt.registry);
      }
      pluginObj.start();
      next();
    });
   ...
   * */
  pluginObj.getIp = thorin.util.getIp.bind(thorin);
};
