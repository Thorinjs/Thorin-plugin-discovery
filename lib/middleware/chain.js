 ...
 * try and execute a remote action once the
...
module.exports = (thorin, opt, pluginObj) => {
  ...
  dispatcher
    ...
    .use((intentObj, next, opt) => {
      ...
      chainId++;
      intentObj.data(`__discoveryChain${intentObj.__chainId}`, opt);
      next();
    })
    .end((intentObj) => {
      if (intentObj.hasError()) return;
      let opt = intentObj.data(`__discoveryChain${intentObj.__chainId}`),
        data;
      if(!opt) return;
